...
class TestHistory:
    ...
    def test_method_list(self, client: SDK4human) -> None:
        history = client.employments.salary_elements.history.list(
            0,
        )
    ...
    def test_raw_response_list(self, client: SDK4human) -> None:
        response = client.employments.salary_elements.history.with_raw_response.list(
            0,
        )
    ...
    def test_streaming_response_list(self, client: SDK4human) -> None:
        with client.employments.salary_elements.history.with_streaming_response.list(
            0,
        ) as response:
...
class TestAsyncHistory:
    ...
    async def test_method_list(self, async_client: AsyncSDK4human) -> None:
        history = await async_client.employments.salary_elements.history.list(
            0,
        )
        assert_matches_type(HistoryListResponse, history, path=["response"])
    ...
    async def test_raw_response_list(self, async_client: AsyncSDK4human) -> None:
        response = await async_client.employments.salary_elements.history.with_raw_response.list(
            0,
        )
    ...
    async def test_streaming_response_list(self, async_client: AsyncSDK4human) -> None:
        async with async_client.employments.salary_elements.history.with_streaming_response.list(
            0,
        ) as response:
            assert not response.is_closed
            assert response.http_request.headers.get("X-Stainless-Lang") == "python"
