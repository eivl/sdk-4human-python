...
__all__ = ["SkillCreateParams", "Field"]


class SkillCreateParams(TypedDict, total=False):
    company_id: Required[Annotated[int, PropertyInfo(alias="companyId")]]

    employment_id: Required[Annotated[Optional[int], PropertyInfo(alias="employmentId")]]

    fields: Required[Iterable[Field]]

    parent_id: Required[Annotated[Optional[int], PropertyInfo(alias="parentId")]]
    """Id of parent skill used in renewed process."""

    privacy: Required[Literal["public", "myOrganization", "myManagers", "onlyMe"]]

    skill_type_id: Required[Annotated[str, PropertyInfo(alias="skillTypeId")]]

    status: Required[Literal["toBePlanned", "planned", "started", "toBeApproved", "approved", "notApplicable"]]
